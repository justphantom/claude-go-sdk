...
// resolveSettingsDir turns the configured settings dir into an absolute path.
...
func resolveSettingsDir(dir string) string {
	...
	// Expand a leading "~/" (or bare "~") to $HOME so values like
...
// every call rescans. An empty settingsDir (HOME unset and Options left empty)
// yields an error so the caller can surface it instead of showing an empty list.
...
// errNoSettingsDir is returned when neither Options nor $HOME could resolve a
...
// nil — the caller surfaces "no settings files found".
