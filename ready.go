...
// timeout bounds the probe so a hung binary does not block startup.
// extraFields are appended to the "ready" log line for backend-specific
// context (e.g. claude's permission_mode).
func checkVersion(ctx context.Context, cliPath, backendName string, timeout time.Duration, logger *slog.Logger, extraFields ...any) error {
	ctx, cancel := context.WithTimeout(ctx, timeout)
	defer cancel()
	// #nosec G204 -- cliPath comes from trusted Options, not user input.
