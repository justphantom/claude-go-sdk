...
// files returns nil, nil (the caller surfaces "no settings files found").
...
func TestListSettings_CacheExpiresAfterTTL(t *testing.T) {
	...
	c := New(Options{SettingsDir: dir, SettingsCacheTTL: time.Second})
...
// TestListSettings_CacheDisabledWhenTTLNegative verifies a negative TTL
// turns caching off: every call rescans. (Zero is not tested here: in the
// SDK zero means "default 1h", not "disabled".)
func TestListSettings_CacheDisabledWhenTTLNegative(t *testing.T) {
	dir := writeSettingsFiles(t, "settings.json")
	c := New(Options{SettingsDir: dir, SettingsCacheTTL: -1})
